...
p_heteromer_cofactors <- ggplot(meta[!is.na(meta$Binding.mode),], aes(BindingMode, Cofactors)) + xlab("Binding mode") +
  geom_violin(fill="#0F6B99") + stat_summary(fun.data = mean_se) + theme_sleek() + coord_cartesian(ylim=c(0,70)) + 
...
p_effector_cofactors <- ggplot(meta[!is.na(meta$effector),], aes(effector, Cofactors)) + 
  geom_violin(fill="#0F6B99") + stat_summary(fun.data = mean_se) + theme_sleek() + xlab("Effector domain(s)") +
...
doSEMplot(meta, order.by = "Cofactors", group.by=by, vars = c("motifIC", "nVICE", "% disorder")) + labs(x="Value", subtitle="CAP characteristics") + scale_x_continuous(n.breaks=3) +
