...
tfs <- row.names(tfs)[which(tfs$isTF)]
...
maxPreds <- maxPreds[,intersect(tfs, colnames(maxPreds))]
```
...
for(f in seq_len(nrow(ds))[!is.na(ds$bestContext) & ds$species!="mouse"]){
  ...
  if(fromScratch){
    ...
    preds <- preds[,colnames(preds) %in% tfs]
...
motifs <- readRDS("motifs.rds")
motifs2 <- readRDS("../00_data_preparation/03_motif_aggregation/AllMotifs_onePerGene.rds")[c("CTCF","MAZ","ZNF143")]
