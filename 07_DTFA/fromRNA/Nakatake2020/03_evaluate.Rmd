...
row.names(tt) <- gsub("^res_", "", row.names(tt))
tt2 <- tt[c("logCPM_predsReweigh.raw", "logCPM_proxReweigh.raw", "logCPM_preds.raw", "logCPM_collecTRI", "logCPM_motifs.raw"),]
tt2$method <- c("Predictions\n(reweighted)+Links", "Predictions (reweighted)\n(Promoters only)", "Predictions\n(raw)+Links", "collecTRI", "Motifs+Links")
