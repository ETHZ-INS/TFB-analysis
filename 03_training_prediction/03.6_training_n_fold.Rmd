---
title: "03.6 Training - n-fold cross-validation"
...
ptm <- proc.time()
nFold <- 4
nHoldOut <- ceiling(length(trainingContexts)/nFold)
for(i in seq(1,length(trainingContexts), nHoldOut)){
  valItContexts <- trainingContexts[i:min((i+nHoldOut-1),length(trainingContexts))]
  
  fmValIt <- fmTrain[which(rowData(fmTrain)$context %in% valItContexts),]
  fmValIt <- fmValIt[sample(1:nrow(fmValIt), nRowPerf*nHoldOut),]  
  ...
  fmValChrsSubIt <- fmValChrsSub[which(!(rowData(fmValChrsSub)$context %in% valItContexts)),]
  ...
  trainInd <- unique(assays(fmTrainSubIt)$features[which(rowData(fmTrainSubIt)$context %in% setdiff(trainingContexts, 
                                                                                                    valItContexts)), "context"])
  prTrainIt[,all_train_labels_check:=paste(trainingContexts[trainInd], collapse=".")]
  prTrainIt[,all_train_labels:=paste(setdiff(trainingContexts, valItContexts), collapse=".")]
  gc()
  ...
  prValIt[,context_label:=trainingContexts[context]]
  valInd <- unique(assays(fmValIt)$features[which(rowData(fmValIt)$context %in% valItContexts), "context"])
  prValIt[,all_holdout_labels_check:=paste(trainingContexts[valInd], collapse=".")]
  prValIt$all_holdout_labels <- paste(valItContexts, collapse=".")
  prValIt[,dist_to_closest_train:=min(mdsDist[,context_label][setdiff(trainingContexts, 
                                                                      valItContexts)]), 
          by=seq_len(nrow(prValIt))]
  gc()
  ...
  prValChrs$all_holdout_labels <- paste(valItContexts, collapse=".")
  valInd <- unique(assays(fmValChrsSubIt)$features[which(rowData(fmValChrsSubIt)$context %in% valItContexts), "context"])
  prValChrs[,all_holdout_labels_check:=paste(trainingContexts[valInd], collapse=".")]
  prValChrs[,dist_to_closest_train:=min(mdsDist[,context_label][setdiff(trainingContexts, 
                                                                        valItContexts)]), 
          by=seq_len(nrow(prValChrs))]
  gc()

  # get prediction table
  prIt <- rbindlist(list(prValChrs, prValIt, prTrainIt), 
                    use.names=TRUE, fill=TRUE)
  prIt$trainset <- paste(setdiff(trainingContexts, valItContexts), collapse=".")
  prIt$holdout_set <- paste(valItContexts, collapse=".")
  ...
  featImpValIt$trainset <- paste(setdiff(trainingContexts, valItContexts), collapse=".")
  featImpValIt$holdout_set <- paste(valItContexts, collapse=".")
  ...
  hpDt$trainset <- paste(setdiff(trainingContexts, valItContexts), collapse=".")
  hpDt$holdout_set <- paste(valItContexts, collapse=".")
