...
p2 <- ggplot(pip, aes(100*prop)) + geom_histogram(fill=tfContextColors[1], boundary=0, closed="right") + theme_bw() + 
  labs(x="% of peaks within PREs", y="Number of CAPs/celltype",
       subtitle=paste0("Across 3811 ENCODE ChIP-seq\nfor ", length(unique(pip$TF)), " CAPs")) +
  xlim(50,100) + 
...
sub <- paste0(">12k ATAC-matched ChIP experiments:\n", length(unique(chipDt$tfName)), 
              " CAPs across ", length(unique(chipDt$context)), " cellular contexts")

p3 <- ggplot(anno, aes(x=n_con)) + geom_histogram(fill=tfContextColors[1], center=1, bins=32) + theme_bw() + 
    labs(x="Number of cellular contexts", y="Number of CAPs", subtitle=sub) + scale_x_sqrt(breaks=c(1,5,15,60,150)) +
...
```{r}
res <- readRDS("../06_TF_meta/motif_chip_overlaps.rds")
tfs <- readRDS("../06_TF_meta/TF_annotation.rds")
tfs <- row.names(tfs)[which(tfs$isLambertTf)]
res2 <- res[res$tf %in% tfs & res$chipThreshold==50 & res$motifLength<40,]
...
p4 <- ggplot(res2[order(res2$motifIC),], aes(both/chip, both/motif, colour=pmin(35,motifIC))) +
  ...
  labs(x="Sensitivity, i.e. proportion of peaks (in PREs) that have the motif", color="Motif\ntotal IC",
       y="Specificity, i.e. the\nproportion of motif matches\n(in PREs) that have a peak",
       subtitle="Not only are most motif matches not bound, but for most TFs a large fraction of peaks fail to show a clear motif")+
  theme_sleek()+
  theme(plot.subtitle=element_text(size=subTitleSize))

# roughly which(res2$both/res2$chip > 0.98 | res2$both/res2$motif>0.2 | (res2$both/res2$chip > 0.98 & as.integer(res2$motifThreshold)==2L) )
tfs <- strsplit("KDM2A, MAX, ZNF761, HMGA1, GRHL2, ZBTB38, FOSL2, NFKB1, YY1, NRF1, ATF2, ZNF131, CGGBP1, MCM3, NFIX, HLF, RLF, ZNF384",", ")[[1]]

p4 <- marginalDistributions(p4) + 
  ggrepel::geom_text_repel(data=res2[res2$tf %in% tfs,], aes(label=tf),
                           color="black", size=2.5, min.segment.length=0)
```
...
plot_grid(
  ...
  nrow=3, rel_heights=c(3,4.6,3.4)
)
dev.off()
```
