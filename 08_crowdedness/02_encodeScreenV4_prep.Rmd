...
Adding distance to links

```{r}
proms <- promoters(EnsDb.Hsapiens.v86, columns="gene_name", upstream = 1L, downstream = 0L)
seqlevels(proms) <- paste0("chr",seqlevels(proms))
medianTssByGene <- median(splitAsList(start(proms), proms$gene_name))
m$dist <- round(abs(start(resize(crwd.gr, fix="center", width=1L))[m$PRE.id] - medianTssByGene[m$gene]))
saveRDS(m, file="PRE2gene_cellLines.rds")
```
