...
if(fromScratch){
  ...
  dhs <- resize(dhs, width=pmin(300L, width(dhs)), fix="center")
...
```{r}
if(fromScratch){
  b <- fread("../broad/archetypes_in_consensus.bed.gz")
...
w <- w[2*seq_len(floor((length(w)-1)/2))]
train <- train[w]
...
```{r}
em <- fread("embeding10")
em <- round(100*as.matrix(as.data.frame(em)))
mode(em) <- "integer"
