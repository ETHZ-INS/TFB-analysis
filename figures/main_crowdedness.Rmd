...
context.crwd <- readRDS("../08_crowdedness/crowdedness_per_context.rds")
...
p_cwd <- ggplot(nTfs, aes(value)) + geom_histogram(bins=27, fill=tfContextColors[1]) + scale_x_sqrt(breaks=b) + 
  ...
  labs(x="Number of CAPs with peaks centers in the same PRE", y="Thousands\nof regions") +
...
p_cwd_pairwise <- GGally::ggpairs(as.data.frame(100*crwd3[which(rowSums(crwd3)>0),]), lower = list(continuous = hex_fun), diag="blank", legend=11, progress=FALSE) +   labs(x="Crowdedness (% of assayed CAPs)", fill="Number of\nregions") + theme(panel.border = element_blank())
...
```{r}
dat1 <- readRDS("../08_crowdedness/crowdedness_vs_ATAC.rds")
...
p_cwd_atac <- ggplot(dat1, aes(ATAC2, crwd2)) + geom_hex(bins=40) +  theme_sleek() +
  ...
  labs(x="Accessibility (ATAC read density/200bp)", y="Crowdedness (% of CAPs)", fill="# of regions") + scale_y_sqrt(breaks=c(1,20,60)) + scale_x_log10(breaks=c(5,150,5000)) + facet_wrap(~context) + coord_cartesian(ylim=c(1,115)) +
...
```{r}
dp2 <- readRDS("../08_crowdedness/crowded_in_promsSEs.rds")
dpn <- attr(dp2, "n")
...
```{r}
m <- readRDS("../08_crowdedness/motifScores_in_crowded_vs_bound_V3.rds")
...
```{r, eval=FALSE}
anno <- readRDS("../06_TF_meta/TF_annotation.rds")
...
```{r}
d3 <- readRDS("../08_crowdedness/crowdedness_GR_rna.rds")
ag1 <- aggregate(d3[,c("log2FC","scaledLFC")], by=d3[,c("sample","timePoint","crowdset")], FUN=\(x) mean(x))
ag2 <- aggregate(d3[,c("log2FC","scaledLFC")], by=d3[,c("sample","timePoint","crowdset")], FUN=\(x) 100*sum(x>0.3)/length(x))
levels(ag1$crowdset) <- levels(ag2$crowdset) <- c(paste0("GR peak\n", levels(ag1$crowdset)[1:3]), "matched\nno GR")

base_pal <- brewer.pal(9, "Blues")
pal20All <- colorRampPalette(base_pal)(20)
pal20 <- pal20All[(length(pal20All)-length(levels(ag1$timePoint))):(length(pal20All)-1)]

p2a <- ggplot(ag1, aes(crowdset, log2FC, fill=timePoint)) + geom_boxplot(outliers = FALSE) + 
  theme_sleek() + labs(y="mean log2FC", x="Promoters",
                       fill="Time after\nactivation") +
  theme(axis.title.x=element_blank(), axis.text.x = element_blank())
p2b <- ggplot(ag2, aes(crowdset, log2FC, color=timePoint)) + stat_summary(position = position_dodge2(width=1)) +
  theme_sleek() + labs(y="% upregulated\n(logFC>0.5)", x="Promoters",
                       color="Time after\nactivation")

p2 <- (p2a / p2b) + plot_layout(guides="collect") & 
  scale_color_manual(values=pal20) & scale_fill_manual(values=pal20) & 
  guides(fill = guide_legend(ncol = 2), color = guide_legend(ncol = 2)) 
```


```{r}
ag <- aggregate(d3[,c("log2FC","scaledLFC")], by=d3[,c("feature","timePoint","crowdset")], FUN=\(x) mean(x))
p2a2 <- ggplot(ag, aes(crowdset, log2FC, colour=timePoint, group=timePoint)) + stat_summary(position = position_dodge2(width=1)) + theme_sleek() + 
  labs(y="log2FC", x="Promoters", colour="Time after\nactivation") +
  theme(axis.title.x=element_blank(), axis.text.x = element_blank())
p2.2 <- (p2a2 / p2b) + plot_layout(guides="collect") & 
  scale_color_manual(values=pal20) & guides(color = guide_legend(ncol = 2)) 

```


```{r}
d <- readRDS("../08_crowdedness/crowdedness_GR.rds")
...
plot_grid(plot_grid(p_cwd, scale=0.95, labels="AUTO"),
  plot_grid(p_cwd_pairwise,
            plot_grid(grid.grabExpr(ComplexHeatmap::draw(upset)), 
                      p_cwd_atac, nrow=2, scale=c(0.9,0.95), rel_heights = c(2,3),
                      labels=c("C","D")),
            nrow=1, rel_widths = c(5,4), scale=c(0.95, 1), labels=c("B", NA)),
  plot_grid(
    plot_grid(p.pp, p1, p5, nrow=3, scale=0.95, labels=c("E","F","H")),
    plot_grid(grid.grabExpr(ComplexHeatmap::draw(h)), 
              p2.2, nrow=2, scale=0.95, labels=c("G","I")),
