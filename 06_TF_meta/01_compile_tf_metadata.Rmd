...
allgenes <- genes(EnsDb.Hsapiens.v86)
ens2sym <- setNames(allgenes$symbol, allgenes$gene_id)
e$symbol <- ens2sym[e$ID]
w <- which(is.na(e$symbol))
e$symbol[w] <- e$Name[w]
row.names(e) <- e$symbol
e$isLambertTf <- e[,4]=="Yes"
e <- e[,c("DBD", "Binding mode", "isLambertTf")]
m <- merge(e,h,by="row.names",all=TRUE)
m$isTF <- m$isLambertTf | m$GRECO_dbTF=="yes"
```
...
d <- readRDS("PRE_distance_to_TSS.rds")
...
anno$isTF[is.na(anno$isTF)] <- FALSE
...
anno$customClass[which(!anno$isTF)] <- "non-TF"
anno$customClass <- relevel(factor(anno$customClass,unique(anno$customClass[order(anno$superclass)])), "non-TF")
```
