...
load(file="../06_TF_meta/performances.RData", verbose=TRUE)
```

Overfitting:

```{r}
print(length(unique(anno$TF)))
...
```{r}
meta <- readRDS("../06_TF_meta/shap_by_meta.SE.rds")
meta <- meta[,colData(meta)$TF %in% anno$TF]
meta <- cbind(as.data.frame(colData(meta)), t(assay(meta)))
meta <- meta[!is.na(meta$ATAC),]
cofactors <- readRDS("../06_TF_meta/shap_cofactors.rds")
cofactors <- subset(cofactors, TF %in% anno$TF)
hp <- readRDS("../06_TF_meta/hyperparams.rds")
hp <- subset(hp, TF %in% anno$TF)
...
# TODO: adapt to complexity defintion of xgboost paper
...
penr1 <- ggplot(mTf, aes(x=auc_pr_mod.test, y=log2(enrichment.test), shape=train.nC2, color=(pos_frac.test))) + 
  ...
  scale_color_viridis_c(trans="sqrt", 
                        guide = guide_colorbar(order=1,
                              ...
                              barheight = unit(3, "mm"),
                              barwidth = unit(40, "mm")),
                        breaks = c(0.001,0.02,0.05)) + #scales::breaks_pretty(n = 4))+
  scale_x_sqrt()+
...
```{r, frac shared peaks}
print(length(unique(mTf$TF)))
pTrain <- ggplot(mTf[!is.na(mTf$auc_pr_mod.test),], aes(x=frac_shared_peaks_test, 
  ...
  geom_smooth(aes(group=1), method=MASS::rlm, linewidth=1.5, color="darkblue") +
  scale_color_viridis_c(trans="sqrt", option="B", breaks=c(0.01,0.15,0.45,0.8)) +
  scale_x_sqrt() + 
...
pdist1 <- ggplot(mTf, aes(frac_shared_peaks_test, log2enrDiff)) + theme_sleek() + 
    ...
    scale_color_viridis_c(option="A", trans="sqrt", breaks=c(0,0.1,0.4,0.8))+scale_x_sqrt()+
...
predAnnotDt <- subset(predAnnotDt, tfName %in% anno$TF)
...
labs(x=TeX("$|hp \\cap \\, tp|/|tp|$", bold=TRUE),
     y=TeX("$|hp \\cap \\, tp|/|hp|$", bold=TRUE),
     fill="Number of CAP x context combinations")+
...
```{r, main perf simplified, eval=FALSE}
pdf("main_performances.pdf", width=12, height=4)
free(ofp+labs(tags="A"))+
free(pnC+labs(tags="B"))+
free(pTrain+labs(tags="C", x="Proportion of non-cell-type-specific peaks"))+
