...
```{r, get Feature-Matrix, eval=trainModel}
ptm <- proc.time()
...
if(length(trainingContexts)==1){
  ...
  saveRDS(featToRemove, file.path(outDir, "featsToRemove.rds"))
}else{
  featToRemove <- c()
  saveRDS(featToRemove, file.path(outDir, "featsToRemove.rds"))
}
...
```{r, split feature matrix by holdout chromosomes, eval=trainModel}
...
```{r, setup feature importance & performance estimates, eval=trainModel}
prAll <- data.table()
...
```{r, feature importance on holdout chromosomes, eval=trainModel}
