...
```{r}
go <- readRDS("../08_crowdedness/GOenrichments_of_crowdedPromoters.rds")
...
se <- readRDS("../08_crowdedness/TFs_cor_with_crowdedness.rds")
...
res <- readRDS("../08_crowdedness/reprodScores_by_crwdBin.rds")
...
hl <- lapply(names(ll), \(n){
  ...
  row_title <- NULL
  if(n==names(ll)[1]) row_title <- "CAPs"
  grid.grabExpr(
...
```{r}
d <- readRDS("../08_crowdedness/reproducibility_by_cutnrun.rds")
...
p.cnr <- ggplot(d, aes(crwd.bin, value, fill=bin.type)) + geom_col(position = position_dodge()) + 
  ...
  labs(y="Proportion of ChIP peaks\nreproduced by Cut&Run", fill="Bin type",
       x="Crowdedness bin (% of assayed CAPs)")
```

## E

```{r}
mo2 <- readRDS("../08_crowdedness/motifDiff_vs_dist2TSS.rds")
